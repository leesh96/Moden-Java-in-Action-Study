...
import static chap02.BehaviorParameterizationSample.Color.GREEN;
import static chap02.BehaviorParameterizationSample.Color.RED;
...
public class BehaviorParameterizationSample {
    ...
    public static void main(String[] args) {
        ...
        List<Apple> greenApples = filterApples(apples, new GreenColorApplePredicate());
    ...
    Predicate?
    직역하면 술어인데, 인수로 값을 받아 참, 거짓을 반환하는 함수를 말한다.
    ...
    */
    ...
    // 이 인터페이스를 구현해서 다양한 프레디케이트를 만들 수 있다.
    ...
    각 알고리즘(전략)을 캡슐화하는 인터페이스를 정의한 다음 런타임에 알고리즘(전략)을 선택하는 방법이다.
    ApplePredicate의 구현체가 런타임에 선택되는 전략이다.
    */

    // 4단계. 추상적 조건으로 필터링 = 동작 파라미터화
    // 필터링 메소드가 다양한 동작을 받아서 내부적으로 다양한 동작을 수행할 수 있다. -> 위에서 구현한 ApplePredicate를 사용하자
    ...
    // 개선 3. 모든 리스트에 대해 필터링이 가능하도록 리스트 형식으로 추상화
